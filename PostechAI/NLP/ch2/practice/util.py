...
def outputWeights(weights, path):
    ...
    out = open(path, 'w')
    for f, v in sorted(weights.items(), key=lambda f_v : -f_v[1]):
        print('\t'.join([str(f), str(v)]))
    out.close()
...
def verbosePredict(phi, y, weights, out):
    ...
    if y:
        ...
        print('Prediction:', yy)
    for f, v in sorted(phi.items(), key=lambda f_v : -f_v[1] * weights.get(f_v[0], 0)):
        w = weights.get(f, 0)
...
def outputClusters(path, examples, centers, assignments):
    ...
    for j in range(len(centers)):
        print('====== Cluster %s' % j)
        print('--- Centers:')
        for k, v in sorted(centers[j].items(), key = lambda k_v : -k_v[1]):
            if v != 0:
                print('%s\t%s' % (k, v))
        print('--- Assigned points:')
        for i, z in enumerate(assignments):
            if z == j:
                print(' '.join(examples[i].keys()))
    out.close()
